...
const SLOT_LABELS: Record<string, string> = {
  mon_afternoon: "周一下午", mon_evening: "周一晚上", mon_late: "周一深夜",
  tue_afternoon: "周二下午", tue_evening: "周二晚上", tue_late: "周二深夜",
  wed_afternoon: "周三下午", wed_evening: "周三晚上", wed_late: "周三深夜",
  thu_afternoon: "周四下午", thu_evening: "周四晚上", thu_late: "周四深夜",
  fri_afternoon: "周五下午", fri_evening: "周五晚上", fri_late: "周五深夜",
  sat_afternoon: "周六下午", sat_evening: "周六晚上", sat_late: "周六深夜",
  sun_afternoon: "周日下午", sun_evening: "周日晚上", sun_late: "周日深夜",
};
...
function scoreLearning(a: ScoreMap, b: ScoreMap, quality = 1): ModuleScore | null {
  ...
  return { score: clamp01(pairs.reduce((sum, value) => sum + value, 0) / pairs.length), coverage: clamp01((pairs.length / 2) * quality), reasons: [] };
}
...
function v1Playstyle(profile: V1Profile): PreferenceMap {
  ...
  const pace: Record<string, Preference> = {
    hardcore: { ideal: 4, tolerance: 0 }, steady: { ideal: 3, tolerance: 1 }, burst: { ideal: 3, tolerance: 1 }, casual: { ideal: 0, tolerance: 1 }, adaptive: { ideal: 2, tolerance: 2 },
  };
...
function aggregateV2ToV1Availability(grid: AvailabilityGrid) {
  ...
  for (const [key, value] of numericEntries(grid)) {
    ...
    if (key.startsWith("sat_") || key.startsWith("sun_")) {
      if (key.endsWith("_afternoon")) tags.add("weekend_day");
  ...
  try {
    ...
    const matches = profiles
      ...
      .map((profile) => {
        ...
        const result = scoreCandidate(own, v2 ?? asV2Bridge(profile), profile, Boolean(v2));
    ...
    return new Response(JSON.stringify({ matches, profileReady: true }), { headers: { ...CORS, "Content-Type": "application/json", "Cache-Control": "no-store" } });
  } catch (error) {
    console.error(error);
